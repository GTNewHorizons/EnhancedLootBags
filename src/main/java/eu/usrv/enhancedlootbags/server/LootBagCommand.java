...
public class LootBagCommand implements ICommand {
    ...
    public void processCommand(ICommandSender pCmdSender, String[] pArgs) {
        try {
            ...
            if (tSubCommand.equalsIgnoreCase("reload")) {
                ...
                else PlayerChatHelper.SendError(pCmdSender, String.format("Reload successful"));
            ...
            if (tSubCommand.equalsIgnoreCase("addloot") || tSubCommand.equalsIgnoreCase("addinventory")) {
                ...
                if (tFlagsOK) {
                    ...
                    if (tGrp != null) {
                        ...
                        if (tSubCommand.equalsIgnoreCase("addloot")) tStacksToAdd.add(inHand.copy());
                        else if (tSubCommand.equalsIgnoreCase("addinventory")) {
                            for (ItemStack is : tEp.inventory.mainInventory)
                                if (is != null) tStacksToAdd.add(is.copy());
                        }

                        for (ItemStack tis : tStacksToAdd) {
                            String tItemID = ItemDescriptor.fromStack(tis).toString();
                            if (tItemID.isEmpty()) continue; // Something went wrong..

                            String tItemNBT = "";
                            if (tis.stackTagCompound != null) tItemNBT = tis.stackTagCompound.toString();

                            Drop dr = tLGF.createDrop(
                                    tItemID,
                                    UUID.randomUUID().toString(),
                                    tItemNBT,
                                    tAmount,
                                    tRandomAmount == 1 ? true : false,
                                    tChance,
                                    tLimitedDropCount,
                                    "");
                            ...
                            PlayerChatHelper.SendInfo(
                                    pCmdSender,
                                    String.format("Item %s added to LootGroup ID %d ", tItemID, tGrp.getGroupID()));
                        }
                    } else
                ...
                if (pArgs.length == 5) {
                    ...
                    if (tIntRarity < 0 || tIntRarity >= EnumRarity.values().length) tFlagsOK = false;
                    else tRarity = EnumRarity.values()[tIntRarity];
                }
                if (tFlagsOK) {
                    LootGroup tGrp = EnhancedLootBags.LootGroupHandler.getGroupByID(tGroupID);
    ...
    private boolean InGame(ICommandSender pCmdSender) {
        if (!(pCmdSender instanceof EntityPlayer)) return false;
        else return true;
    }
